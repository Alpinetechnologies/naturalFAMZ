...
export default function DeliveredOrders(props) {
  const deliverorder = [
    {deliver: 'success', status: 'Delivered by Mohit Ray'},
    {status: 'Customer was unavailable'},
    {deliver: 'success', status: 'Delivered by Raj Ag'},
  ];
  const itemlist = [{}, {}];
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {deliverorder.map((data, j) => (
          <View style={styles.card} key={j}>
            ...
            {data.deliver === 'success' ? (
              <View style={styles.deliverbox}>
                <Text style={styles.deliverBy}>{data.status}</Text>
              </View>
            ) : (
              <View style={styles.undeliverbox}>
                <Text style={styles.deliverBy}>{data.status}</Text>
              </View>
            )}
          </View>
